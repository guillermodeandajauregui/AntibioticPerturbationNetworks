# run_sweep_WS_raw.R
# Watts–Strogatz (small-world) sweep; save compact sim outputs per sim; write an index table.
#
# Saves per sim:
...
# ---- override helpers (WS ids + totals) --------------------------------------
# Keep your original sweep_helpers.R intact; override only in this script.

make_graph_id <- function(n, k, beta, graph_seed_id) {
  # encode beta as integer percent to keep filenames clean (e.g. 0.10 -> b010)
  b <- as.integer(round(100 * beta))
  sprintf("n%04d_k%02d_b%03d_g%04d", n, k, b, graph_seed_id)
}

n_graph_total <- function(n_values, k_values, beta_values, n_graph) {
  length(n_values) * length(k_values) * length(beta_values) * n_graph
}
...
# WS: each node initially connected to k neighbors (k must be even)
k_values <-
  c(4, 8, 12)

# WS rewiring probability (beta)
beta_values <-
  c(0.01, 0.05, 0.10)
...
N_graph_total <-
  n_graph_total(n_values, k_values, beta_values, n_graph)
...
for (n in n_values) {
  for (k in k_values) {
    ...
    if (k %% 2 != 0) stop("WS requires k to be even; got k = ", k)
    
    # igraph::sample_smallworld uses 'nei' (neighbors each side); degree ~ 2*nei
    nei <- as.integer(k / 2)
    ...
    for (beta in beta_values) {
      for (graph_rep in seq_len(n_graph)) {
        ...
        graph_id <-
          make_graph_id(n, k, beta, graph_seed_id)
        
        # WS graph reproducible with global seed id 1..N
