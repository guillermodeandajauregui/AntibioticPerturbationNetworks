# plot_ba_topology_delta_heatmaps_full.R
...
df_sum <-
  ...
  mutate(
    ...
    k0_frac = factor(k0_frac, levels = sort(unique(k0_frac))),
...
make_delta_heatmap <-
  function(data,
    ...
    ggplot(plot_data, aes(x = k0_frac, y = p, fill = mean_value)) +
      ...
      labs(
        title = paste0(title_text, " (N = ", as.character(n_value), ")"),
